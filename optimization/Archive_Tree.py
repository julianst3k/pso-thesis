...
class ArchiveTreeController:
    ...
    def __init__(self, evaluated_values, particles, hypercubes=None):
    ...
    def create_tree(self):
        ...
        for i, val in enumerate(self.values):
            tree.add_value(val, 0, i, self.particles[i,:])
        return tree
...
class ArchiveTree:
    ...
    def add_value_node(self, node):
        ...
        self.add_value(value, key, index, particle)

    def add_value(self, value, key, index, particle):
        if self.root is None:
            ...
            if self.top is not None and self.size >= self.top:
                ...
                val = self.root.add(value, key, index, particle)
            self.size += val
        else:
            ...
            if new_root is not None:
                self.root = new_root
                self.size += new_root.add(value, key, index, particle)
            else:
                self.root = ArchiveTreeNode(value, key, index, particle)
                self.size = 1
...
class ArchiveTreeNode:
    ...
    def add(self, value, key, index, particle, root = False):
        ...
        if self.not_dominated(value):
            if value[key] < self.get_key_value():
                if self.get_left() is None:
                    new_node = ArchiveTreeNode(value, key, index, particle)
                    self.set_left(new_node)
                    return 1
                else:
                    return self.get_left().add(value, key, index, particle)
            else:
                ...
                if self.get_right() is None:
                    ...
                    return self.get_right().add(value, key, index, particle)
        else:
