...
class TestPSO:
    ...
    def test_optimizer_ic_check_rep(self):
        ...
        assert all([n_two.get_particle()[0] <= n_one.get_particle()[0] or n_two.get_particle()[1] <= n_one.get_particle()[1] for n_two in nodes_two for n_one in nodes])

    def test_optimizer_iterator(self):
        numero_de_particulas = 100
        dimension = 2
        parameters = {0: {"lb": 0, "ub": 1}, 1: {"lb": 0, "ub": 1}}
        turbulence = 0.0001
