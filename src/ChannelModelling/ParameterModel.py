...
class ModelSeed(sim.Model):
    def __init__(self, betat, betar, fov, alphat_outside, alphat_inside, coord_rec):
        ...
        for i in range(4):
            if i == 0 or i == 3:
                alphat = alphat_outside*(-1)**i+90
            else:
                alphat = alphat_inside*(-1)**i+90
...
class FitnessCalculator:
    ...
    def calculate_fitness(self, num_rot=8, debug = False):
    ...
    def calculate_non_dominant(self, input_array, noise):
        ...
        for matrix in input_array:
            ...
            sinr_one = [(h1**2 / (h_two[i]**2 + noise), h_two[i]**2 / (h1**2 + noise)) for i, h1 in enumerate(h_one)]
            queue = SolQueue()
